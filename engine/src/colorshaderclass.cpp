...
bool ColorShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	bool result;
	// @TODO: temp allocator
	wchar_t vsFilename[512];
	wchar_t psFilename[512];
	int error;

	// Set the filename of the vertex shader.
	error = wcscpy_s(vsFilename, 512, L"data/shaders/color_vs.hlsl");
	if(error != 0)
	{
		return false;
	}
	...
	error = wcscpy_s(psFilename, 512, L"data/shaders/color_ps.hlsl");
	if(error != 0)
	{
		return false;
	}
...
void ColorShaderClass::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, WCHAR* shaderFilename)
{
	...
	MessageBoxW(hwnd, L"Error compiling shader.  Check ~shader-error.txt~ console for message.", shaderFilename, MB_OK);
